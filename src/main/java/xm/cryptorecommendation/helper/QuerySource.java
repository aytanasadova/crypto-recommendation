...
public class QuerySource {
    ...
    public static final String LIST_ALL_WITH_OLDEST_PRICE =
                                                         " select   d. timestamp,    " +
                                                         "          d.symbol,        " +
                                                         "          d.price          " +
                                                         " from  crypto_data d       " +
                                                         " join  (SELECT   symbol, min(timestamp) timestamp  FROM CRYPTO_DATA  group by symbol ) cc   on     " +
                                                                                                                              "     cc.timestamp=d.timestamp " +
                                                                                                                              " and cc.symbol=d.symbol;";


    public static final String LIST_ALL_WITH_NEWEST_PRICE =
                                                        " select   d. timestamp,  " +
                                                        "          d.symbol,      " +
                                                        "          d.price        " +
                                                        " from  crypto_data d     " +
    ...
    public static final String GET_OLDEST_PRICE_FOR_CRYPTO =
                                                        " select d.timestamp, " +
                                                        "        d.symbol,    " +
                                                        "        d.price      " +
                                                        " from  crypto_data d " +
                                                        " join (SELECT symbol, min(timestamp) timestamp  FROM CRYPTO_DATA  where symbol=? group by symbol ) cc  on " +
                                                                                                                              "     cc.timestamp=d.timestamp       " +
                                                                                                                              " and cc.symbol=d.symbol;            ";
    public static final String GET_NEWEST_PRICE_FOR_CRYPTO =
                                                        " select d.timestamp, " +
                                                        "        d.symbol,    " +
                                                        "        d.price      " +
                                                        " from  crypto_data d " +
                                                        " join (SELECT symbol, max(timestamp) timestamp  FROM CRYPTO_DATA  where symbol=? group by symbol ) cc on   " +
                                                                                                                               "    cc.timestamp=d.timestamp        " +
                                                                                                                               " and cc.symbol=d.symbol;            ";
    ...
    public static final String LIST_ALL_WITH_OLDEST_PRICE_BY_INTERVAL =
                                                        " select   d. timestamp," +
                                                        "          d.symbol,    " +
                                                        "          d.price      " +
                                                        " from  crypto_data d   " +
